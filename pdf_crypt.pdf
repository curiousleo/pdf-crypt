...
input_file = (
    fd.askopenfilename(title="Choose PDF file to decrypt or encrypt", **file_options)
    if len(sys.argv) < 2
    else sys.argv[1]
)
try:
    with pikepdf.Pdf.open(input_file) as pdf:
        ...
        output_file = fd.asksaveasfilename(
            title="Choose where to save encrypted PDF file", **file_options
        )
    ...
    while True:
        try:
            ...
            with pikepdf.Pdf.open(input_file, password=password) as pdf:
                output_file = fd.asksaveasfilename(
                    title="Choose where to save decrypted PDF file", **file_options
                )
